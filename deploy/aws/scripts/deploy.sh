...
#
# Safe to run as many times as you like. Called once by cloud-init on
# first boot; after that you run it by hand:
#
...
SCRIPTS_DIR="$SRC_DIR/deploy/aws/scripts"

# -----------------------------------------------------------------
# Infrastructure values, written by cloud-init from Terraform.
# -----------------------------------------------------------------
...
# Secrets.
#
# Read straight into shell variables via jq. Never written anywhere
# except the compose .env file, which is chmod 600.
# -----------------------------------------------------------------
...
SMTP_USER="$(jqs SMTP_USER)"
SMTP_PASS="$(jqs SMTP_PASS)"
META_APP_SECRET="$(jqs META_APP_SECRET)"
META_APP_ID="$(jqs META_APP_ID)"

if [ -z "$JWT_SECRET" ]; then
  echo "FATAL: JWT_SECRET is empty in $SECRET_ID" >&2
  exit 1
fi
...
SUPABASE_ANON_KEY="$("$SCRIPTS_DIR/gen-jwt.sh" "$JWT_SECRET" anon)"
SUPABASE_SERVICE_ROLE_KEY="$("$SCRIPTS_DIR/gen-jwt.sh" "$JWT_SECRET" service_role)"
...
# every value here comes from Terraform or Secrets Manager and is
# overwritten on the next run. Change the source, then re-run.
...
PSQL=(psql --host "$DB_HOST" --port "$DB_PORT" --username postgres --dbname "$DB_NAME" --no-password)
...
for attempt in $(seq 1 60); do
  ...
  if [ "$attempt" -eq 60 ]; then
    ...
    echo "Check the RDS instance state and the db security group." >&2
    exit 1
  fi
  sleep 5
done
...
if [ "$PULL" -eq 1 ]; then
  ...
  compose pull kong auth rest realtime storage
fi
